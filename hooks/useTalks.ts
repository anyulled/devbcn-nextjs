...
export const getTalks = cache(async (year: string | number = "default", options?: { strict?: boolean }): Promise<SessionGroup[]> => {
  try {
    const url = getSessionsUrl(year);

    const response = await fetch(url, getSessionizeFetchOptions(year));
    if (!response.ok) {
      const errorMessage = `Failed to fetch talks for year ${year}: ${response.statusText}`;
...
export const getTalkSpeakersWithDetails = async (year: string | number, speakerIds: string[]): Promise<Speaker[]> => {
  ...
  /*
   * ⚡ Bolt: Convert array to Set for O(1) membership checks.
   * Reduces time complexity of lookup from O(N * M) to O(N + M)
   * where N is the number of speakers and M is the number of speakerIds.
   */
