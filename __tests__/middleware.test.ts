...
describe("middleware", () => {
  it("serves a cleanup script for legacy registerSW.js requests", async () => {
    const response = middleware(createRequest("https://www.devbcn.com/registerSW.js"));
  ...
  it("serves a legacy manifest for manifest.json requests", async () => {
    const response = middleware(createRequest("https://www.devbcn.com/manifest.json"));
    const body = await response.text();
  ...
  it("rewrites service-worker.js to the kill-switch worker", () => {
    const response = middleware(createRequest("https://www.devbcn.com/service-worker.js"));
