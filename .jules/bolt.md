...
## 2024-08-04 — Optimize tag parsing in static pages

**Learning:** Found a common performance issue in Next.js static generation where nested traversals (e.g., `allTalks.flatMap().find()`) create heavy memory allocations during `generateStaticParams` and metadata generation.
**Action:** Replace `flatMap().find()` with nested `for...of` loops or `.some()` combined with early exit logic. This avoids O(N^2) allocations and GC overhead when processing hundreds of elements.
