...
## 2024-08-29 - Avoid array extraction with maximum limit using full traversal

**Learning:** Using `.filter(condition).slice(0, limit)` forces a full array traversal and allocates a discarded intermediate array.
**Action:** Use `.find()` or a standard `for...of` loop with a `break` to exit early once the required limit is reached to improve performance and avoid overhead.
