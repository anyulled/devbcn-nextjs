...
## 2026-11-20 - Avoid array.flatMap().find() for O(N) traversals

**Learning:** Chaining `.flatMap().find()` to locate an item in nested arrays creates a flattened array, iterating multiple times and adding unnecessary memory overhead.
**Action:** Use `.find()` with an internal `.some()` to locate the parent element, then extract the needed child value.
