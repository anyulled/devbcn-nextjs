...
export default async function Page({ params }: { params: Promise<{ tag: string }> }) {
  ...
  const filteredTalks = allTalks.filter((talk) => {
    ...
    return talkTags.some((t) => t.replaceAll(" ", "-").toLowerCase() === normalizedDecodedTag);
  });

  const displayTag = filteredTalks[0]
    ? (getTagsFromTalk(filteredTalks[0]).find((t) => t.replaceAll(" ", "-").toLowerCase() === normalizedDecodedTag) ?? decodedTag.replaceAll("-", " "))
    : decodedTag.replaceAll("-", " ");
