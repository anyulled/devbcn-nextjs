...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const userIdByEmail = authUsersPage.users.reduce<Map<string, string>>((map, user) => {
    if (user.email) {
  ...
  if (sponsorUpdateError) {
    return NextResponse.json({ error: sponsorUpdateError.message }, { status: 500 });
  }

  const { error: contactsDeleteError } = await supabase.from("sponsor_users").delete().eq("sponsor_id", parsedId.data);

  if (contactsDeleteError) {
    return NextResponse.json({ error: contactsDeleteError.message }, { status: 500 });
  }
