...
export async function generateMetadata({ params }: Readonly<TagPageProps>): Promise<Metadata> {
  ...
  const sessionGroups = await getTalks(year);
  const allTalks = sessionGroups.flatMap((group) => group.sessions);
  const matchedTalk = allTalks.find((talk) => getTagsFromTalk(talk).some((t) => t.replaceAll(" ", "-").toLowerCase() === decodedTagLower));
  const displayTag = matchedTalk
    ? (getTagsFromTalk(matchedTalk).find((t) => t.replaceAll(" ", "-").toLowerCase() === decodedTagLower) ?? decodedTag.replaceAll("-", " "))
...
export default async function TagPage({ params }: Readonly<TagPageProps>) {
  ...
  const displayTag = filteredTalks[0]
    ? (getTagsFromTalk(filteredTalks[0]).find((t) => t.replaceAll(" ", "-").toLowerCase() === decodedTagLower) ?? decodedTag.replaceAll("-", " "))
    : decodedTag.replaceAll("-", " ");
