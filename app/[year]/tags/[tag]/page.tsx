...
export async function generateMetadata({ params }: Readonly<TagPageProps>): Promise<Metadata> {
  ...
  const matchedTalk = allTalks.find((talk) => getTagsFromTalk(talk).some((t) => t.replaceAll(" ", "-").toLowerCase() === targetTagLower));
  const displayTag = matchedTalk
    ? (getTagsFromTalk(matchedTalk).find((t) => t.replaceAll(" ", "-").toLowerCase() === targetTagLower) ?? decodedTag.replaceAll("-", " "))
