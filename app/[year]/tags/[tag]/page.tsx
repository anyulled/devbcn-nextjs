...
export async function generateMetadata({ params }: Readonly<TagPageProps>): Promise<Metadata> {
  ...
  const displayTag =
    (matchedTalk ? getTagsFromTalk(matchedTalk).find((t) => t.replaceAll(" ", "-").toLowerCase() === targetTag) : undefined) ?? decodedTag.replaceAll("-", " ");
...
export default async function TagPage({ params }: Readonly<TagPageProps>) {
  ...
  const displayTag =
    (filteredTalks[0] ? getTagsFromTalk(filteredTalks[0]).find((t) => t.replaceAll(" ", "-").toLowerCase() === targetTag) : undefined) ??
