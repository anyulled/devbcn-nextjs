...
export async function generateMetadata({ params }: Readonly<TagPageProps>): Promise<Metadata> {
  ...
  const matchingTalk = allTalks.find((talk) => getTagsFromTalk(talk).some((t) => t.replaceAll(" ", "-").toLowerCase() === targetTagLower));
  const displayTag = matchingTalk
    ? (getTagsFromTalk(matchingTalk).find((t) => t.replaceAll(" ", "-").toLowerCase() === targetTagLower) ?? decodedTag.replaceAll("-", " "))
...
export default async function TagPage({ params }: Readonly<TagPageProps>) {
  ...
  const filteredTalks = allTalks.filter((talk) => getTagsFromTalk(talk).some((t) => t.replaceAll(" ", "-").toLowerCase() === targetTagLower));

  const displayTag = filteredTalks[0]
    ? (getTagsFromTalk(filteredTalks[0]).find((t) => t.replaceAll(" ", "-").toLowerCase() === targetTagLower) ?? decodedTag.replaceAll("-", " "))
    : decodedTag.replaceAll("-", " ");
